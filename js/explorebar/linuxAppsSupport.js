...
var enableBlurOnLinuxApps = false;  //disable this for perfomance
...
var keepCheckingForMinimized = true;
			...
			if (currentDesktopX != deskNox) {
				...
				/*executeNativeCommand('xdotool getactivewindow', function (currentActive, error) {
					...
					if (nextGenericProcessId != null) {
						...
						executeNativeCommand('wmctrl -i -R '+nextFilesProcessId);
						executeNativeCommand('wmctrl -i -R '+nextGenericProcessId);
					}
				});*/
				loadWIndows(true);
			} else {
...
var maximizedWindows = [];
var maximizedWindowsVerified = [];
var currentlyProcessingPos = 0;
...
function checkIfMinimized() {
	...
	//console.log("maximize in here2: ",currentlyProcessingPos);
	//console.log("maximizedWindows: ",maximizedWindows);
		if (maximizedWindows.length > currentlyProcessingPos) {
		//console.log("maximize in here3");
			executeNativeCommand("xprop -id "+maximizedWindows[currentlyProcessingPos].id+" | grep 'window state: Iconic'", function (data, err) {
			//console.log("do we even get here?");
				if (!err) {
				//console.log("minimized data: ",data);
					if (data == "")
						maximizedWindowsVerified.push(maximizedWindows[currentlyProcessingPos]);
				} else {
				//console.log("maxmimize err, adding:");
				maximizedWindowsVerified.push(maximizedWindows[currentlyProcessingPos]);
				}
				currentlyProcessingPos++;
				checkIfMinimized();
			});
		} else {
...
//console.log("so.....");
	maximizedWindows = allActiveWindows.filter(function (windowX) {
  		return windowX.height > (window.screen.height-100) && windowX.yOffset < 50 && windowX.workspace == currentDesktopX;
	});
	
	//console.log("maximizedWindowsA: ",maximizedWindows);
	keepCheckingForMinimized = true;
	currentlyProcessingPos = 0;
	maximizedWindowsVerified = [];
	checkIfMinimized();
...
function loadWIndows(forceLoadChanges) {
	keepCheckingForMinimized = false;
